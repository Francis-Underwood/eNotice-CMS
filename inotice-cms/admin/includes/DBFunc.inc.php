<?php
require_once('DButility.inc.php');
require_once('DB_reset.inc.php');
...
function Save_config_title($data) {
	...
	//Color
	$sql = "UPDATE global_config SET `config_str`=".GetSQLValueString(strtoupper($data['company_title_color']),"text")." ";
	...
	//Color
	$sql = "UPDATE global_config SET `config_str`=".GetSQLValueString(strtoupper($data['company_subtitle_color']),"text")." ";
	...
	//Current Time Color
	$sql = "UPDATE global_config SET `config_str`=".GetSQLValueString(strtoupper($data['curr_time_color']),"text")." ";
	...
	//Weather temp Color
	$sql = "UPDATE global_config SET `config_str`=".GetSQLValueString(strtoupper($data['weather_temp_color']),"text")." ";
...
function Save_casting($data) {
	...
	$sql = "UPDATE global_config SET `config_str`='<font color=\'#".strtoupper($data['casting_color'])."\'>".$casting_text."</font>' ";
...
function del_about_us_image($id) {
	if ($id > 0) {
		$sql = "UPDATE global_config SET `config_str`='' WHERE (`module` = 'about_us' AND `name` = 'about_us_img' AND `id` = ".$id.") ";
		//echo $sql;
		$result = mysql_query($sql);
		if (!($result)) {
			return false;
...
function del_news_image($img_id) {
	if ($img_id > 0) {
		$sql = "UPDATE news_pic SET `deleted` = 1 WHERE ((`id` = ".$img_id."))";
		$result = mysql_query($sql);
		if (!($result)) {
			return false;
		}
...
function Select_banner_image($id) {
	...
	$query_rsview .= "ORDER BY `sorting_id`, `id` ASC ";
...
function del_gallery_one_photo($aid, $id) {
	if ($id > 0) {
		$sql = "UPDATE gallery SET `deleted` = 1 WHERE ((`aid` = $aid) AND (`id` = $id))";
		$result = mysql_query($sql);
		if (!($result)) {
			return false;
		}
...
//Save <li></li> ordering to db
function Save_sorted_items($subject, $sorted_id_array)   {
	$tb = "";
	switch ($subject) {
		case "banner_image":
			$tb = "banner_image";
	...
	tolog("tb:   ".$tb);
	if ($tb != "")  {
		 for ($i = 0; $i < (count($sorted_id_array)); $i++) {
			$sql = "UPDATE $tb SET `sorting_id`=$i ";
			$sql .= "WHERE ((`deleted` = 0) AND (id = ".$sorted_id_array[$i].")) ";
...
function Select_images($subject, $image_id, $id2) {
	...
	If (!($query_rsview == "")) {
		...
		if ($totalRows_rsview >0 ){
			...
			do {
				$data['thumb_image'][$count] = $config_data['folder']."/preview.jpg";
				$data['image'][$count] = $config_data['folder']."/preview.jpg";
				$data['no_image'][$count] = true;
...
function Update_images($subject, $image_id, $image_file_name, $org_file_name, $id2)   {
	...
	//Fill the sorting_id same as id
	if (!(fill_sorting_id( $subject, mysql_insert_id() )))   {
		return false;
	}
...
//Fill the sorting_id which is same as generated ID
function fill_sorting_id($subject, $pid)  {
	$tb="";
	switch ($subject) {
		case "banner_image":
			$tb = "banner_image";
	...
	if ($tb != "")  {
		$sql = "UPDATE $tb SET `sorting_id` = $pid WHERE ((`deleted` = 0) AND (`id` = $pid)) ";
		$result = mysql_query($sql);
		if (!($result)) {
			return false;
		}
