...
<div class="mws-panel grid_8">
	<div class="mws-panel-header">
		...
		<span class="ids-vince-right-header-span">Total Gallery Videos: <?php echo $movie_gallery_count."&nbsp;"; ?></span>
	</div>
	<div class="mws-panel-body">
		...
		<form class="mws-form" id="ids-vince-custom-order-form" method="POST" action="scripts/order_handler.php">
			<?php if (($movie_gallery_count) > 0) {  ?>
				<ul id="mws-gallery" class="clearfix">
					<?php for ($i = 0; $i <= ($movie_gallery_count - 1); $i++) {   ?>
						<li id="<?php echo $movie_gallery_data['id'][$i]; ?>" style="cursor:move;">
							<?php output_image($config_data['folder']."/thumb/".$movie_gallery_data['thumb'][$i],80,0,"","Thumb Err!"); ?></br>
							<strong><?php echo $movie_gallery_data['title'][$i]; ?></strong>
						</li>
					<?php }	?>	
				</ul>
			<?php  }   
